...
class DatalakeSyncService {
  constructor(db, dashdocConnector, options = {}) {
    ...
    this.config = {
      ...
      // Rate limiting
      rateLimitDelayMs: options.rateLimitDelay || 500,
      maxConcurrentRequests: options.maxConcurrent || 2,
  ...
  async runPeriodicSync() {
    ...
    try {
      console.log('[DATALAKE] Running periodic sync...');
  ...
  async runFullSync() {
    ...
    try {
      ...
      for (const { name, fn } of syncOrder) {
        try {
          console.log(`[DATALAKE] Syncing ${name}...`);
          await fn();
          await this.delay(this.config.rateLimitDelayMs);
        } catch (error) {
