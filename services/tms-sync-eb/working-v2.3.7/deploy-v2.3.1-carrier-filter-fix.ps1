# Script pour créer un package déployable v2.3.1 - Fix filtre carriers/shippers
Write-Host "Creating deployment package v2.3.1 - Carrier filter fix..." -ForegroundColor Cyan

$sourceDir = "c:\Users\rtard\dossier symphonia\rt-backend-services\services\tms-sync-eb"
$tempDir = "C:\temp\tms-sync-v2.3.1"
$zipPath = "$sourceDir\deploy-v2.3.1-carrier-filter-fix.zip"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "Package v2.3.1 created successfully!" -ForegroundColor Green
    ...
    Write-Host "  - Added is_shipper=false filter to exclude shippers" -ForegroundColor White
    Write-Host "  - Filter out account_type='invited' companies" -ForegroundColor White
    Write-Host "  - Fixes '1 UP' (donneur d'ordre) being shown as carrier" -ForegroundColor White
    Write-Host "`nDeploy at:" -ForegroundColor Yellow
    Write-Host "https://eu-central-1.console.aws.amazon.com/elasticbeanstalk/home?region=eu-central-1#/environment/dashboard?applicationName=rt-tms-sync-api-v2&environmentId=e-uqxwcfszym" -ForegroundColor Gray
} else {
